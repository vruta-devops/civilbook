...
class ServiceController extends Controller
{
    public function getServiceList(Request $request){
        ...
        $apiLogData = [
            ...
            'api_response' => '',
            'api_name' => 'API-service-list'
        ];
        ApiLog::insert($apiLogData);
        $service = Service::where('service_type', 'service')->with(['providers', 'category', 'serviceRating', 'uploadedRequiredCertificates'])->orderBy('created_at', 'desc');
    ...
    public function getServiceDetail(Request $request){
        ...
        if(auth()->user() !== null){
            if(auth()->user()->hasRole('admin')){
                $service = Service::where('service_type', 'service')->withTrashed()->with('providers', 'category', 'serviceRating', 'serviceAddon', 'slots', 'certificates.certificate')->findorfail($id);
            }
            else{
                $service = Service::where('service_type', 'service')->with('providers', 'category', 'serviceRating', 'serviceAddon', 'slots', 'certificates.certificate')->findorfail($id);
            }
        }else{
            $service = Service::with('providers', 'category', 'serviceRating', 'serviceAddon', 'slots', 'certificates.certificate')->where('service_type', 'service');
        ...
        $related = $service->where('service_type', 'service')->where('user_service_status', 1)->where('category_id', $service->category_id);
         if(default_earning_type() === 'subscription'){
        ...
        foreach ($tax as $val) {
            $taxes[] = [
                ...
                'title' => optional($val->taxes)->title,
                'type' => optional($val->taxes)->type,
                'value' => !empty($val->taxes) && !empty($val->taxes->value) ? (double)$val->taxes->value : 0,
            ];
        }

        if (!empty($service_detail->tax)) {
            $taxes[] = [
                'id' => 0,
                'provider_id' => $request->provider_id,
                "title" => "Other Tax",
                "type" => "percent",
                "value" => empty($service_detail->tax) ? 0 : doubleval($service_detail->tax)
            ];
        }
        ...
        $providers = UserResource::collection(User::whereIn('id',$service->serviceProviderMapping->pluck('provider_id'))->get());
        $response = [
    ...
    public function deleteFavouriteService(Request $request)
    {
        ...
        $service_rating = UserFavouriteService::where('user_id', $request->user_id)->whereIn('service_id', $request->service_id)->delete();
