...
class PaymentController extends Controller
{
    public function savePayment(Request $request)
    {
        ...
        $service = Service::where('id', $booking->service_id)->select('is_enable_advance_payment')->first();

        if (!empty($service) && $service->is_enable_advance_payment == 1) {
    ...
    public function paymentList(Request $request)
    {
        ...
        if($request->has('datetimefrom') && !empty($request->datetimefrom)){
            $payment->where('datetime','>=',date('Y-m-d', strtotime($request->datetimefrom)));
        }
        if($request->has('datetimeto') && !empty($request->datetimeto)){
            $payment->where('datetime','<=',date('Y-m-d', strtotime($request->datetimeto)));
        }
    ...
    public function paymentDetail(Request $request){
        ...
        if (auth()->user()->hasAnyRole(['handyman']) || auth()->user()->hasAnyRole(['provider'])) {
            $get_all_payments = $get_all_payments->where('receiver_id',$user_id)->where(function ($query) {
                if (auth()->user()->hasAnyRole(['handyman']) && !auth()->user()->hasAnyRole(['provider'])) {
                    return $query->where('action' ,'handyman_approved_cash');
                } else if (!auth()->user()->hasAnyRole(['handyman']) && auth()->user()->hasAnyRole(['provider'])) {
                    return $query->where('action' ,'handyman_send_provider');
                } else if (auth()->user()->hasAnyRole(['handyman']) && auth()->user()->hasAnyRole(['provider'])) {
                    return $query->where('action' ,'handyman_send_provider')->orWhere('action', 'handyman_approved_cash');
                } else {
                    return $query;
                }
            });
