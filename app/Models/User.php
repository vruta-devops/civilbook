...
class User extends Authenticatable implements HasMedia
{
    ...
    protected $fillable = [
        ...
        'known_languages', 'skills', 'description', 'why_choose_me', 'department_id', 'preferred_location_distance', 'provider_grade', 'area'
    ];
    ...
    protected $with = ['providerAddresses'];
    ...
    {
        return $this->hasMany(ProviderSlotMapping::class, 'provider_id', 'id')->where('status', 1);
    }
    ...
    {
        return $this->hasMany(ProviderAddressMapping::class, 'provider_id', 'id')->where('status', 1);
    }
    ...
    public function subscriptionPackage(){
        return $this->hasOne(ProviderSubscription::class, 'user_id', 'id')->where('status', config('constant.SUBSCRIPTION_STATUS.ACTIVE'))->where('end_at', '>=', Carbon::now()->toDateString());
    }
    ...
    public function providerCategoryMapping(){
        return $this->hasMany(ProviderCategoryMapping::class, 'provider_id','id')->with('category','subCategory');
    }

    public function providerServiceMapping()
    {
        return $this->hasMany(ServiceProviderMapping::class, 'provider_id', 'id');
    }

    public function providerCommonServiceMapping()
    {
        return $this->hasMany(ServiceProviderMapping::class, 'provider_id', 'id')->with('services')
            ->whereHas('services', function ($q) {
                return $q->where('admin_service_type', 'common');
            });
    }
    /**
     * The badges that belong to the provider.
     */
    public function badges()
    {
        return $this->belongsToMany(Badges::class, 'badge_provider');
    }

    public function providerBadgeMapping(){
        return $this->hasMany(BadgeProvider::class, 'provider_id','id');
    }

    public function providerCurrentBadgeMapping()
    {
        return $this->hasOne(BadgeProvider::class, 'provider_id', 'id')->orderBy('id', 'desc');
    }
