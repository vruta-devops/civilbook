...
function checkCustomPermission($permission, $session = null): bool
{
    if (empty($session)) {
        $session = authSession();
    }

    return collect($session->getAllPermissions())->contains('name', $permission);
}
...
function storeMediaFile($model, $attachedFile, $name)
{
    ...
    if ($attachedFile) {
        ...
        if (is_array($attachedFile)) {
            /*
            for($m=0;$m<count($attachedFile);$m++){
                echo " <<<<<< !!!  ".$attachedFile[$m]." !!! >>>>> ";exit;
                $model->addMedia($attachedFile[$m])->toMediaCollection($name);
            }
            */
            // echo '<pre>';
            foreach ($attachedFile as $uploadedValue) {
                // print_r($uploadedValue);
                if (!empty($uploadedValue)) {
                    // echo "AHERE ";exit;
                    $model->addMedia($uploadedValue)->toMediaCollection($name);
                }
            }
        } else {
            $model->addMedia($attachedFile)->toMediaCollection($name);
        }
    }
...
function saveBookingActivity($data)
{
    ...
    switch ($data['activity_type']) {
        ...
        case "update_booking_status":
            ...
            $status = BookingStatus::bookingStatus($data['booking']->status);
            $old_status = BookingStatus::bookingStatus($data['booking']->old_status);
        ...
        case "cancel_booking":
            ...
            $activity_data = [
                'reason' => $data['booking']->reason,
                'status' => $data['booking']->status,
                'status_label' => BookingStatus::bookingStatus($data['booking']->status),
            ];
            $sendTo = removeArrayValue(['admin', 'provider', 'handyman', 'user'], $role);
            break;
...
function getPriceFormat($price)
{
    ...
    $currency_symbol = Setting::where('type', 'CURRENCY')->where('key', 'CURRENCY_COUNTRY_ID')->with('country')->first();
    ...
    $currency_position = Setting::where('type', 'CURRENCY')->where('key', 'CURRENCY_POSITION')->first();
    $position = 'left';
...
function dateAgoFormate($date, $type2 = '')
{
    ...
    $diff_time = Carbon::parse($diff_date)->tz('Asia/Kolkata')->isoFormat('LLL');
...
function timeAgoFormate($date)
{
    ...
    $timestampInIST = Carbon::createFromFormat('Y-m-d H:i:s', $date);

    return $timestampInIST->timezone('Asia/Kolkata')->diffForHumans();
}
...
{
    $setting_data = Setting::where('type', $type)->where('key', $key)->first();
...
{
    $documents = Documents::where('is_required', 1)->where('status', 1)->withCount([
        'providerDocument',
...
function get_handyman_provider_commission($handyman_id)
{
    ...
    $get_commission = HandymanType::withTrashed()->where('id', $hadnymantype_id)->first();
    if ($get_commission) {
...
{
    $revenuedata = Payment::selectRaw('sum(total_amount) as total , booking_id, DATE_FORMAT(datetime , "%m") as month')
...
function savePayoutActivity($data)
{
    ...
    foreach ($sendTo as $to) {
        switch ($to) {
            case 'provider':
                $user = User::getUserByKeyValue('id', $data['user_id']);
                break;
            case 'handyman':
                $user = User::getUserByKeyValue('id', $data['user_id']);
                break;
        ...
        $user->notify(new PayoutNotification($notification_data));
    }
...
{
    $get_provider_plan = ProviderSubscription::where('user_id', $user_id)->where('status', config('constant.SUBSCRIPTION_STATUS.ACTIVE'))->first();
    $activeplan = null;
...
{
    $user_subscribed = ProviderSubscription::where('user_id', $user_id)->where('status', config('constant.SUBSCRIPTION_STATUS.ACTIVE'))->first();
    $value = 0;
    if ($user_subscribed) {
        $value = 1;
    }
...
function check_days_left_plan($old_plan, $new_plan)
{
    ...
    $previous_plan_end = new Carbon($old_plan->end_at);
...
{
    $user_subscribed = ProviderSubscription::where('user_id', $user_id)
...
{
    $gettype = AppSetting::first();
    $earningtype = !empty($gettype) && $gettype->earning_type ? $gettype->earning_type : 'commission';
    return $earningtype || null;
}
...
function saveWalletHistory($data)
{
    ...
    $notification_data = [
        'id' => empty($data['wallet']) ? "" : $data['wallet']->id,
        'type' => empty($data['activity_type']) ? "" : $data['activity_type'],
        'subject' => empty($data['activity_type']) ? "" : $data['activity_type'],
        'message' => empty($data['activity_message']) ? "" : $data['activity_message'],
        'notification_type' => 'wallet',
    ];
    $user = User::getUserByKeyValue('id', $data['wallet']->user_id);

    notificationSend($user, $data['activity_type'], $notification_data);
}
    ...
    if (is_any_plan_active($provider_id) == 1) {
        ...
        if ($get_current_plan->plan_type === 'limited') {
            ...
            if ($type === 'service') {
                ...
                $provider_service_count = Service::where('provider_id', $provider_id)->whereDate('created_at', '>=', $plan_start_date)->count();
            ...
            if ($type === 'featured_service') {
                ...
                $provider_featured_service_count = Service::where('provider_id', $provider_id)->where('is_featured', 1)->whereDate('created_at', '>=', $plan_start_date)->count();
                if ($limit_array['is_checked'] == 'on' && $limit_array['limit'] != null) {
            ...
            if ($type === 'handyman') {
                ...
                $handyman_count = User::where('provider_id', $provider_id)->whereDate('created_at', '>=', $plan_start_date)->count();
                if ($limit_array['is_checked'] == 'on' && $limit_array['limit'] != null) {
...
function sendNotification($type, $user, $data): void
{
...
function saveNotification($notificationData, $userId)
{
    $notificationData['notification-type'] = $notificationData['type'];
...
function saveAdminNotification($notificationData)
{
    $adminUsersIds = User::where('user_type', 'admin')->pluck('id');

    if (!empty($adminUsersIds)) {
        foreach ($adminUsersIds as $adminUsersId) {
            saveNotification($notificationData, $adminUsersId);
        }
    }
}
...
function saveRequestJobActivity($data)
{
    ...
    $radius = 50;
    $providers = ProviderAddressMapping::selectRaw("id, provider_id, address, latitude, longitude,
...
function getServiceTimeSlot($provider_id)
{
    ...
    $handyman_count = User::where('provider_id', $provider_id)->where('is_available', 1)->count() + 1;

    $providerSlots = ProviderSlotMapping::where('provider_id', $provider_id)
        ->whereIn('days', $days)
...
function today_cash_total($user_id, $to = '', $from = '', $type = '')
{
    ...
    if (auth()->user()->hasAnyRole(['handyman'])) {
        $amount = PaymentHistory::where('receiver_id', $user_id)
...
{
    $name = '';
    $user = User::getUserByKeyValue('id', $user_id);
    if ($user !== null) {
...
{
    $payment_status_check = PaymentHistory::where('payment_id', $payment_id)
        ->where('action', 'handyman_send_provider')->where('status', 'pending_by_admin')->first();
    if ($payment_status_check !== null && auth()->user()->hasRole('admin')) {
...
{
    $payment_status_check = PaymentHistory::orderBy('id', 'desc')->where('payment_id', $payment_id)->first();
...
function providerpayout_rezopayX($data)
{
    ...
    $rezorpay_data = PaymentGateway::where('type', 'razorPayX')->first();
    ...
    if ($rezorpay_data) {
        ...
        $currency_country_data = Setting::where('type', 'CURRENCY')->first();

        $currency_country = json_decode($currency_country_data, true);

        $currency_country_id = $currency_country['value'];

        $country_data = Country::where('id', $currency_country_id)->first();
        ...
        $providers_details = User::where('id', $provider_id)->first();
...
function providerpayout_stripe($data)
{
    ...
    $stripe_data = PaymentGateway::where('type', 'stripe')->first();
...
function getTotalRequests($userId)
{
    return Booking::where('customer_id', $userId)->count();
}

function getTotalExpiredRequests($userId)
{
    return Booking::where('customer_id', $userId)->whereDate('date', '<', now()->toDateString())->whereNotIn('status', ['completed', 'cancelled'])->count();
}

function getTotalOrders($userId)
{
    return Booking::where('customer_id', $userId)->whereNotIn('status', ['pending', 'cancelled'])->count();
}

function getTotalAcceptedOrders($userId)
{
    return Booking::where('customer_id', $userId)->where('status', 'accept')->count();
}

function infoLog($message, $value = [])
{
...
function googleMatrixApi($userLatitude,$userLongitude,$destinationLat,$destinationLng, $distanceReq){
    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=$userLatitude,$userLongitude&destinations=$destinationLat,$destinationLng&key=AIzaSyBIXHOQdGu6rdxWmu0HdUEqCJS3W5IFtfk";

    $response = file_get_contents($url);
    $data = json_decode($response);
    if ($data->status == 'OK') {
        // Extract distance in meters (you can convert it to other units as needed)
        ...
        $accurateDistance = $distanceReq; // Handle API error
    }
    return $accurateDistance;
}

function notificationSend($user, $type, $notificationData,)
{
    try {
        $notificationResponse = $user->notify(new CommonNotification($type, $notificationData, $user->user_type));
        infoLog("Notification response", $notificationResponse);
        saveNotification($notificationData, $user->id);
    } catch (CouldNotSendNotification $exception) {
        Log::error('Notification could not be sent: ' . $exception->getMessage());
        // You can also log additional details if needed
    }
}

function checkEmpty($variable, $key, $defaultValue = "-")
{
    if (empty($variable) || empty($variable[$key]) || $variable[$key] == 'null') {
        return $defaultValue;
    }
