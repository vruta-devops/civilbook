            ...
            @foreach($notifications->sortByDesc('created_at')->take(5) as $notification)
                <li class="dropdown-item-1 float-none p-3 {{ $notification->read_at ? '':'notify-list-bg'}} ">
                    @php
                        ...
                        $route = match ($type) {
                            'new_service_document_uploaded' => route('providerdocument.index'),
                            'new_provider_register' => route('provider.pending', 'pending'),
                            'new_service_create' => route('service.pending-service'),
                            'new_post_job_create' => route('post-job-request.show', $notification->data['id']),
                            'new_Chat_Message' => route('chat'),
                            default => route('booking.show', $notification->data['id']),
                        };
                    @endphp
                    <a href="{{$route }}" class="">
