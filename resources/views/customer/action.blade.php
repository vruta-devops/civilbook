...
<div class="d-flex justify-content-end align-items-center">
    @if(!$user->trashed())
        ...
        @if($auth_user->can('user edit'))
                @php
                    $url = route('user.create',['id' => $user->id]);

                    if (!empty($listStatus)) {
                        $url = route('user.create',['id' => $user->id, 'status' => $listStatus]);
                    }
                @endphp
            ...
            @if($auth_user->can('user delete'))
                <a class="mr-2 text-danger" href="{{ route('user.destroy', $user->id) }}"
                   data--submit="user{{$user->id}}"
