...
(function($) {
    ...
    $(document).ready(function(){
        ...
        window.addEventListener('load', function() {
            const menuLink = $(".side-menu>li.active");
            const scollElement = $(".scroll-content");
            console.log(menuLink.length);
            // Get the element's top offset
            var menuLinkOffset = menuLink.offset();
            var topOffset = menuLinkOffset.top;
            // Get the window height
            var windowHeight = window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight;
            if (topOffset > windowHeight) {
                console.log('Element is below the window');
                const scrollValue = topOffset - windowHeight;
                const transformValue = scrollValue + 55 ;
                const transCss = "translate3d(0," + -transformValue + "px,0)";
                console.log(transCss);
                 scollElement.css("transform", transCss);
            } else if (topOffset < 0) {
                console.log('Element is above the window');
            }
        });
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.status == false){
                        ...
                        $("#userstatus" + id).prop('checked', !status);
                    }else{
        ...
        function getChatNotificationCounts() {
            var url = "{{ route('chat-notification.counts') }}";
            $.ajax({
                type: 'get',
                url: url,
                success: function (res) {
                    if (res.counts > 0) {
                        $('.chat_notify_count').addClass('notification_tag').text(res.counts);
                        setChatNotification(res.counts);
                        $('.chat_notification_list span.dots').addClass('d-none')
                        $('.chat_notify_count').removeClass('d-none')
                    } else {
                        $('.chat_notify_count').addClass('d-none')
                        $('.chat_notification_list span.dots').removeClass('d-none')
                    }
        ...
        setInterval(getChatNotificationCounts, 10000);
        ...
        function setChatNotification(count) {
            console.log("count================", count);
            if (Number(count) >= 100) {
                $('.chat_notify_count').text('99+');
            }
        }
